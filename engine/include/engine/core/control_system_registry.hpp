...
// Singleton factory that maps ControlSystemID strings to IControlSystem
// factory functions.
//
...
public:
    ...
    void register_system(ControlSystemID id, FactoryFn factory);

    // Create a new instance of the named control system.
    // Throws std::out_of_range if the ID is not registered.
    std::unique_ptr<IControlSystem> create(const ControlSystemID& id) const;

    // Returns true if the ID is registered.
    bool has(const ControlSystemID& id) const;
    ...
    static bool register_static(ControlSystemID id, FactoryFn factory);
