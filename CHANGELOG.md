...
- Same scripts were also missing `libice-dev`/`libsm-dev`, needed by
  qtbase's `sessionmanager`/`xcb-sm` features — masked locally by packages
  already present from earlier, unrelated work on this machine
- `symulator_deploy_qt_conf()` failed CMake's generate step on Windows with
  "Files to be generated by multiple different commands": `qt_add_executable()`
