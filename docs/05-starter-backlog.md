...
- [x] Define minimal database model. **Resolved:** one PostgreSQL instance, two schemas — `fleet` (static reference data) and `session` (events, snapshots, EDR live, operating point assignments, chat). Details in `11-database-model.md`.
- [ ] Prepare one reference traffic scenario for testing. **Scope defined:** two operating points — Gdynia Orłowo and Sopot, plus the line section between them. Blocked on: Station Editor and Scenario Linter (Layer 1) must be functional before topology can be validated. Architecture defined in `13-scenario-editor-architecture.md`. Deferred to Stage 1 completion.
...
- [x] Confirm final station list (include or exclude Naterki) and define default station-to-player assignment. **Resolved:** 9 Trójmiasto stations (Gdynia Chylonia → Gdańsk Orunia); Naterki excluded from MVP (post-MVP). Station assignment is **flexible** — a player (client) receives a list of operating points regardless of geographic station; the client displays one pulpit tab and one EDR tab per assigned operating point and switches freely between them. No fixed geographic split.
- [x] Define station ownership and takeover protocol (request, grant, revoke). **Resolved:** lifecycle — Request: C→S `TAKEOVER_REQUEST(operating_point_id)`; Grant: S→C `TAKEOVER_RESPONSE(granted=true)` + broadcast `OperatingPointOwnershipChanged`; Voluntary release: `TAKEOVER_REQUEST(release=true)` or graceful disconnect; Server revoke: after 30 s grace period on disconnect → `OperatingPointOwnershipChanged(owner=null)`. Details in `09-communication-contract.md`.
