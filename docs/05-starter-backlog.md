...
- [ ] Prepare one reference traffic scenario for testing. **Scope defined:** two posterunki — Gdynia Orłowo and Sopot, plus the line section between them. Blocked on: Station Editor and Scenario Linter (Layer 1) must be functional before topology can be validated. Architecture defined in `13-scenario-editor-architecture.md`. Deferred to Stage 1 completion.
